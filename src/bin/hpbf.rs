...
fn print_help_text() {
    ...
    #[cfg(all(not(miri), target_arch = "x86_64", target_family = "unix"))]
    println!("   --print-jit-mc   Print machine code generated by the JIT to stdout");
    #[cfg(all(not(miri), feature = "llvm"))]
    ...
    #[cfg(all(not(miri), target_arch = "x86_64", target_family = "unix"))]
    println!("   --base-jit       Use the baseline JIT compiler");
    #[cfg(all(not(miri), feature = "llvm"))]
...
fn main() {
    ...
    #[cfg(not(all(not(miri), target_arch = "x86_64", target_family = "unix")))]
    {
    ...
    for arg in env::args().skip(1) {
        if next_is_file {
            ...
            match arg.as_str() {
                ...
                #[cfg(all(not(miri), target_arch = "x86_64", target_family = "unix"))]
                "--print-jit-mc" => kind = ExecutorKind::PrintMc,
                #[cfg(all(not(miri), target_arch = "x86_64", target_family = "unix"))]
                "--base-jit" => kind = ExecutorKind::BaseJit,
                #[cfg(all(not(miri), feature = "llvm"))]
